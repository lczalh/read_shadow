  ...
  */
  dynamic get({@required String baseUrl, @required String path, Map<String, dynamic> params, int delay = 3, int max = 3}) async {
    try {
      ...
      cz_print(response, StackTrace.current);
      ...
      cz_print(e.toString(), StackTrace.current);
      if (max == 1) {
  ...
  /*
    ...
    param 请求失败回调
  */
  post({@required String baseUrl, @required String path, Map<String, dynamic> params, int delay = 3, int max = 3}) async {
    try {
      Response response = await _dio.post("$baseUrl$path", queryParameters: params);
      if (response.statusCode == 200) {
        return json.decode(response.toString());
      } else {
        throw Exception("服务器异常${response.statusCode}");
